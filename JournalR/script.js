let CardMaker = {
  ...
  MakeCard: function (heading, body) {
    ...
    RemButton.setAttribute(
      "onclick",
      "CardMaker.RemoveCard(this.parentElement.id)"
    );
  ...
  ChangeDisplay: function (id) {
    if (localStorage.getItem(id) !== null) {
      document.getElementById("main-journal-header").textContent = JSON.parse(
        localStorage.getItem(id)
      )["h"];
      document.getElementById("main-journal-body").textContent = JSON.parse(
        localStorage.getItem(id)
      )["t"];
    }
